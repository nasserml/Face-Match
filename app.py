...
# save upload image
def save_upload_image(uploadimge):
    try:
        create_directory(dirs=[uploadn_path])
        
        with open(os.path.join(uploadn_path, uploadimge.name),'wb') as f:
            f.write(uploadimge.getbuffer())
...
# streamlit
st.title('To whome does your face match')
uploadimge = st.file_uploader('chose an image')

if uploadimge is not None:
    if save_upload_image(uploadimge):
        #load image
        display_image= Image.open(uploadimge)
        
        # extract feature
        
        
        #features = extract_features(os.path.join(uploadimge, uploadimge.name), model, detector)
        features = extract_features(os.path.join(uploadn_path, uploadimge.name), model, detector)
        #rcommend
